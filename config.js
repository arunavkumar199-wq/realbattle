// 🔥 REALBATTLE CONFIG - Mobile Optimized
// ⚠️ NEECHE APNA SUPABASE URL AUR KEY DALEIN

const SUPABASE_URL = 'https://azxnvrbjqszzcuekcbqs.supabase.co'; // ← Yahan apna URL
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF6eG52cmJqcXN6emN1ZWtjYnFzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE2MTU3NzgsImV4cCI6MjA4NzE5MTc3OH0.5onT-qjNZCXCWts-i79rGylroTOLk57yMoyEb-EwzYI'; // ← Yahan apni anon key
...
// 🎯 10 GUNS - Free Fire Exact Stats (Damage, Range, Reload)
const GUNS = {
...
// 🔫 Damage Calculation (Distance + Headshot)
function calcDmg(gun, distM, isHead) {
  ...
  if (gun.spread > 1) {
    const hits = Math.floor(Math.random() * (gun.spread - 2)) + 2;    dmg = Math.round(dmg * hits / gun.spread * 1.8);
  }
...
function gpsDist(la1,lo1,la2,lo2) {
  if(!la1 || !la2) return 5; // Default 5m if no GPS
  const R=6371000, dL=(la2-la1)*Math.PI/180, dO=(lo2-lo1)*Math.PI/180;
  const a=Math.sin(dL/2)**2+Math.cos(la1*Math.PI/180)*Math.cos(la2*Math.PI/180)*Math.sin(dO/2)**2;
  return R*2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a));
}

// 🧭 Bearing Calculator (Direction between two points)
...
// ✅ Aiming Check: Kya player enemy pe aim kar raha hai?
function isAiming(mLa,mLo,mHd,tLa,tLo,tol=18) {
  if(!mLa || !tLa) return true; // No GPS = always hit (for testing)
  return angDiff(mHd, gpsBear(mLa,mLo,tLa,tLo)) <= tol;
}

// 🎯 Headshot Detection (Pitch + Distance based)
function isHeadshot(pitch,distM) {
  const ideal = Math.atan2(1.7, Math.max(1,distM)) * 180/Math.PI;
  return Math.abs(pitch - ideal) < 22;
}

// 🔊 Tap Sound + 📳 Vibration Feedback
function tap(f=800) {
  try {
